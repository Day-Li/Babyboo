...
exports.login = async function (req, res) {
    ...
    multichain.listStreamKeyItems({stream: userstream, key: username, verbose: false, count: 1, start:-1,},(err, data) => {
	   logger.info("Fetch data from",userstream,"for the key to login",username);
           if (data.length > 0 && data[0].data.txid)
                 {
                       var txid = data[0].data.txid
		       logger.info("Fetch data from",userstream,"for the txid to login",txid);
			...
			//	   }
			 //  ]}
			   //    console.log("Big data output",items)
			   resolve(data)
                	})
                 }
         else {
		 console.log("small data output",data)
           resolve(data)
      	 }
    })
})
    await fetch_pass.then(function (value) {
	    ...
	    {
		    var password_act = hex_to_ascii(value);
	    }
